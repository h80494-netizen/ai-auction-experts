...
CSV_PATH = os.path.join(os.path.dirname(__file__), '../../data/숙박업위치정보.csv')
DB_PATH = os.path.join(os.path.dirname(__file__), '../data/map_data.db')

def process_lodgings():
    print("Loading Lodgings CSV...")
    try:
        # Try cp949 first (common for Korean public data), fallback to utf-8
        try:
    ...
    # Headers are: LDGS_CD, LDGS_NM, LDGS_LA, LDGS_LO
    lat_col = 'LDGS_LA'
    lng_col = 'LDGS_LO'

    if lat_col not in df.columns or lng_col not in df.columns:
        print(f"Error: Required columns {lat_col} and {lng_col} not found in CSV.")
        return
    ...
    print("Running DBSCAN clustering (radius: 200m, min_samples: 5)...")
    ...
    print(f"Found {num_clusters} lodging clusters.")
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS lodging_polygons (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            count INTEGER,
            coordinates_json TEXT
        )
    ''')
    cursor.execute('DELETE FROM lodging_polygons')
    
    for poly in polygons:
        cursor.execute(
            'INSERT INTO lodging_polygons (count, coordinates_json) VALUES (?, ?)',
